...
const STORAGE_KEY = "digitalMarketingDashboard:data:v11";
...
function heroContent() {
  ...
  if (state.view === "seo") {
    ...
    return {
      ...
      description: "Google Search Console, keyword, website post, and Google Business Profile performance for the selected monthly or quarterly view.",
      stats: [
...
function renderPipelinePanel(metrics = overviewMetrics()) {
  return `
    <article class="executive-panel compact-executive">
      <div class="panel-header"><h3>Pipeline Flow</h3><span class="fine-print">${escapeHtml(selectedPeriodLabel())} · ${formatNumber(metrics.totalLeads)} total</span></div>
...
function renderCompareView() {
  ...
  const metrics = [
    ["Spend", "spend", "money", "Total marketing investment."],
    ["Marketing leads", "leads", "number", "Paid, organic, social, and website attributed leads."],
    ["Total leads", "totalLeads", "number", "All leads, including referrals."],
    ["Qualified", "qualified", "number", "Sales-ready lead count."],
    ["In progress", "inProgress", "number", "Qualified leads not yet won or lost."],
    ["Won", "won", "number", "Closed deals from marketing."],
    ["Revenue", "revenue", "money", "Closed revenue."],
    ["Pipeline value", "pipelineValue", "money", "Estimated value of qualified open opportunities."],
    ["CPQL", "cpql", "money", "Cost per qualified lead."],
  ];
  ...
  return `
    <section class="section-header compare-title">
      <div>
        ...
        <p class="definition">A complete comparison of acquisition, social engagement, paid ads, search visibility, pipeline, and revenue.</p>
      </div>
    </section>
    <section class="compare-grid">
      ${renderCompareColumn(left, state.quarter, metrics)}
      ${renderCompareColumn(right, state.compareQuarter, metrics)}
      <article class="panel compare-delta">
        <h3>Change</h3>
        ${metrics
          .map(([label, key, type]) => {
            const diff = parseNumber(left[key]) - parseNumber(right[key]);
            const pct = right[key] ? percentChange(parseNumber(left[key]), parseNumber(right[key])) : null;
            const formatted = type === "money" ? formatMoney(diff, key === "cpql") : formatNumber(diff);
            return `<div class="delta-row"><span>${escapeHtml(label)}<small>${pct === null ? "No baseline" : `${pct >= 0 ? "+" : ""}${formatPct(pct, 1)}`}</small></span><strong class="${diff >= 0 ? "up" : "down"}">${diff >= 0 ? "+" : ""}${formatted}</strong></div>`;
          })
          .join("")}
      </article>
    </section>
    <section class="compare-section-grid">
      ${renderCompareMetricPanel("Social", socialLeft, socialRight, [["Views", "views", "number"], ["Reach", "reach", "number"], ["Engagements", "engagements", "number"], ["Link clicks", "linkClicks", "number"], ["Posts", "posts", "number"]])}
        ...
        <table>
          ...
          <tbody>
            ${metrics.map(([label, key, type]) => {
              ...
              const displayLeft = type === "money" ? formatMoney(leftValue, true) : type === "pct" ? formatPct(leftValue, 2) : type === "number" && key === "avgPosition" ? Number(leftValue || 0).toFixed(1) : formatNumber(leftValue);
              const displayRight = type === "money" ? formatMoney(rightValue, true) : type === "pct" ? formatPct(rightValue, 2) : type === "number" && key === "avgPosition" ? Number(rightValue || 0).toFixed(1) : formatNumber(rightValue);
              const diffText = type === "money" ? formatMoney(diff, true) : key === "avgPosition" ? Number(diff || 0).toFixed(1) : formatNumber(diff);
              const good = key === "avgPosition" ? diff <= 0 : diff >= 0;
              return `<tr><td><strong>${escapeHtml(label)}</strong></td><td>${displayLeft}</td><td>${displayRight}</td><td><span class="${good ? "up" : "down"}">${diff >= 0 ? "+" : ""}${diffText}</span></td><td>${pct === null ? "No baseline" : `${pct >= 0 ? "+" : ""}${formatPct(pct, 1)}`}</td></tr>`;
            }).join("")}
          </tbody>
        </table>
      </div>
...
function renderSocialView() {
  ...
  return `
    ...
    <section class="kpi-grid social-kpis">
      ...
      ${metricCard("Posts", formatNumber(totals.posts), "Post count for the selected period.", totals.posts)}
    </section>
...
function renderSocialCompareRows(rows, compareRows) {
  ...
  const compare = {
    ...
    posts: sum(compareRows, "posts"),
  };
  const metrics = [["Views", "views", "number"], ["Reach", "reach", "number"], ["Engagements", "engagements", "number"], ["Posts", "posts", "number"]];
  ...
  return `
    <article class="panel platform-chart-panel">
      ...
      <div class="chart-wrap chart-watermark">
        ...
        ${lineChart(row.series, "label", "contentViews", "reach", { firstLabel: "Content views", secondLabel: "Reach", height: 300 })}
      </div>
    </article>
  `;
}
...
function renderSocialTable(rows) {
  return `
    <div class="table-wrap compact-table">
      <table>
        <thead><tr><th>Platform</th><th>Views</th><th>Reach</th><th>Engagements</th><th>Link clicks</th><th>Posts</th></tr></thead>
        <tbody>
          ${rows
            .map(
              (row) => `
...
function renderMetaView() {
  ...
  return `
    ...
    <section class="panel">
      <div class="panel-header"><h3>Campaign Breakdown</h3></div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Campaign</th><th>Impressions</th><th>Reach</th><th>Link clicks</th><th>Spend</th><th>Leads</th><th>Qualified</th></tr></thead>
          <tbody>${(meta.campaigns || [])
            .map(
              (row) =>
                `<tr><td><strong>${escapeHtml(row.name)}</strong></td><td>${formatNumber(row.impressions)}</td><td>${formatNumber(row.reach)}</td><td>${formatNumber(row.linkClicks)}</td><td>${formatMoney(row.spend, true)}</td><td>${formatNumber(row.leads)}</td><td>${formatNumber(row.qualified)}</td></tr>`,
            )
            .join("")}</tbody>
        </table>
      </div>
...
function renderMetaTrendPanel(title, meta, firstKey, secondKey, firstLabel, secondLabel) {
  ...
  return `
    <article class="panel">
      ...
      ${metricHeaderRow(metrics)}
      <div class="chart-wrap">${dualAxisLineChart(meta.series || [], "label", firstKey, secondKey, { firstLabel, secondLabel, height: 300 })}</div>
    </article>
  `;
}
...
function renderSeoView() {
  ...
  return `
    ...
    <section class="kpi-grid">
      ...
      ${metricCard("Website posts", formatNumber(seo.posts), "Posts or website content updates made in the period.", seo.posts)}
    ...
    <section class="panel">
      <div class="panel-header"><h3>Published SEO Posts</h3><span class="badge">${formatNumber(seo.posts)} posts</span></div>
      ${renderSeoPostTable(seo.postRows || [])}
    </section>
    <section class="panel">
      <div class="panel-header"><h3>SEO Changes Made</h3><span class="badge">${formatNumber(seo.posts)} SEO posts</span></div>
      <p>${escapeHtml(seo.changesMade)}</p>
    </section>
  `;
}
  ...
  return `
    ...
    <section class="grid-2">
      <article class="panel">
        ...
        <div class="chart-wrap">${lineChart(weeks.map((row) => ({ label: shortDate(row.week), primary: row.leads, secondary: row.qualified })), "label", "primary", "secondary", { firstLabel: "Leads", secondLabel: "Qualified", height: 300 })}</div>
      </article>
      <article class="panel">
        <div class="panel-header"><h3>Weekly Revenue & Won Deals</h3></div>
        <div class="chart-wrap">${lineChart(weeks.map((row) => ({ label: shortDate(row.week), revenue: row.revenue, won: row.won })), "label", "revenue", "won", { firstLabel: "Revenue", secondLabel: "Won deals", height: 300 })}</div>
      </article>
    </section>
    <section class="panel">
...
function bindBaseEvents() {
  ...
  document.querySelector("#keywordSearch")?.addEventListener("input", (event) => {
    ...
    state.keywordSearchTimer = setTimeout(render, 250);
  });
...
function aggregateLeadRows(rows) {
  ...
  rows.forEach((row) => {
    const status = (row.Status || "Uncategorized").trim();
    const statusLower = status.toLowerCase();
    const source = (row.Source || "Uncategorized").trim();
    const channel = (row["Channel Group"] || "Uncategorized").trim();
    const month = monthLabel(row["Date Added"]);
    const qualified = parseNumber(row["Qualified?"]) > 0 ? 1 : 0;
    const won = String(row["Won?"] || "").toLowerCase() === "checked" ? 1 : 0;
    ...
    const quarter = row.Quarter || quarterFromDate(row["Date Added"]) || state.quarter;
    ...
    if (won) {
      wonDeals.push({
        client: row.Name || "Closed deal",
        source: channel || source,
        revenue,
        date: toIsoDate(row["Date Added"]),
        label: shortDate(row["Date Added"]),
        quarter,
        month,
      });
    }
...
function orderedLeadStatuses(statuses, metrics = {}) {
  const value = (...keys) => keys.reduce((total, key) => total + parseNumber(statuses?.[key]), 0);
  return [
    { key: "new", label: "New lead", value: value("New Lead", "For Phone Reach Out") },
    { key: "answered", label: "Answered / qualifying", value: value("Answered/Qualifying") },
    { key: "followUp", label: "Follow ups", value: value("Follow Up Email 1 Sent", "Follow Up Email 2 Sent", "Did Not Answer") },
    { key: "booking", label: "Booking", value: value("Meeting Booked") },
    { key: "qualified", label: "Qualified", value: Math.max(value("Qualified/Added to CoConstruct"), parseNumber(metrics.totalQualified || metrics.qualified)) },
    { key: "won", label: "Won", value: Math.max(value("Won"), parseNumber(metrics.won)) },
    { key: "lost", label: "Lost", value: Math.max(value("Lost"), parseNumber(metrics.lost)) },
    { key: "unqualified", label: "Leads not qualified", value: value("Unqualified") },
  ];
}
...
function dualAxisLineChart(rows, labelKey, firstKey, secondKey, options = {}) {
  ...
  function dots(key, max, color, label, moneyValue = false) {
    return rows.map((row, index) => {
      const x = padding.left + (rows.length <= 1 ? 0 : (index / (rows.length - 1)) * innerWidth);
      const y = padding.top + innerHeight - (parseNumber(row[key]) / max) * innerHeight;
      const value = moneyValue ? formatMoney(row[key], true) : formatNumber(row[key]);
      return `<circle cx="${x}" cy="${y}" r="3" fill="${color}"><title>${escapeHtml(row[labelKey])} · ${escapeHtml(label)}: ${value}</title></circle>`;
    }).join("");
  }
  const grid = [0, 0.5, 1].map((ratio) => {
    const y = padding.top + innerHeight - ratio * innerHeight;
    return `<line x1="${padding.left}" y1="${y}" x2="${width - padding.right}" y2="${y}" stroke="rgba(84,72,69,.12)" /><text x="${padding.left - 12}" y="${y + 4}" text-anchor="end" font-size="12" fill="#544845">${formatNumber(Math.round(maxA * ratio))}</text><text x="${width - padding.right + 12}" y="${y + 4}" font-size="12" fill="#544845">${formatMoney(maxB * ratio, true)}</text>`;
  }).join("");
  const xLabels = rows.map((row, index) => {
    if (index !== 0 && index !== rows.length - 1 && index % xStep !== 0) return "";
    const x = padding.left + (rows.length <= 1 ? 0 : (index / (rows.length - 1)) * innerWidth);
  ...
  return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Dual axis line chart">
    ...
    ${dots(firstKey, maxA, "#c12400", options.firstLabel || firstKey)}
    ${dots(secondKey, maxB, "#544845", options.secondLabel || secondKey, ["cpm", "cpc", "amountSpent"].includes(secondKey))}
    ${xLabels}
  </svg>`;
}
